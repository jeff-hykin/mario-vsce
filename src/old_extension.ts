...
export function activate(context: vscode.ExtensionContext) {
    context.subscriptions.push(vscode.commands.registerCommand("mario.moveUp", () => {
        const editor = vscode.window.activeTextEditor;
        markSelection(editor, nextPosition(editor.document, editor.selection.active, true));
    }));

    context.subscriptions.push(vscode.commands.registerCommand("mario.moveDown", () => {
        const editor = vscode.window.activeTextEditor;
        markSelection(editor, nextPosition(editor.document, editor.selection.active, false));
    }));

    context.subscriptions.push(vscode.commands.registerCommand("mario.selectUp", () => {
        const editor = vscode.window.activeTextEditor;
        markSelection(
            editor,
    ...
    context.subscriptions.push(vscode.commands.registerCommand("mario.selectDown", () => {
        const editor = vscode.window.activeTextEditor;
        markSelection(
            editor,
